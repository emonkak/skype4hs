...
setTopic :: (MonadIO m, MonadSkype m) => ChatID -> ChatTopic -> Skype m ()
...
addMembers :: (MonadIO m, MonadSkype m) => ChatID -> [UserID] -> Skype m ()
addMembers _ [] = return ()
...
-- | Joins to a chat.
joinChat :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ()
...
-- | Leaves to a chat.
leaveChat :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ()
...
getTimestamp :: (MonadIO m, MonadSkype m) => ChatID -> Skype m Timestamp
...
getAdder :: (MonadIO m, MonadSkype m) => ChatID -> Skype m UserID
...
getStatus :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ChatStatus
...
getAllPosters :: (MonadIO m, MonadSkype m) => ChatID -> Skype m [UserID]
...
getAllMembers :: (MonadIO m, MonadSkype m) => ChatID -> Skype m [UserID]
...
getTopic :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ChatTopic
...
getActiveMembers :: (MonadIO m, MonadSkype m) => ChatID -> Skype m [UserID]
...
getWindowTitle :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ChatWindowTitle
...
getAllMessages :: (MonadIO m, MonadSkype m) => ChatID -> Skype m [ChatMessageID]
...
getRecentMessages :: (MonadIO m, MonadSkype m) => ChatID -> Skype m [ChatMessageID]
...
isBookmarked :: (MonadIO m, MonadSkype m) => ChatID -> Skype m Bool
...
-- | Create a chat.
createChat :: (MonadIO m, MonadSkype m) => [UserID] -> Skype m (ChatID, ChatStatus)
...
-- | Open a chat window.
openChat :: (MonadIO m, MonadSkype m) => ChatID -> Skype m ()
